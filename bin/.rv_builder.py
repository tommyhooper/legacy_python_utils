...
#p.add_option("-e",dest='encoder', default='h264',type='string',help="encoder type: h264, prores, or mpeg2 (default = h264)")
...
def get_sequences(files):
	...
	for _file in files:
		...
		#regx = re.search('^(|.*[^0-9])([0-9]+)([^0-9]*)$',full_filename)
		if groups:
	...
	for key in info:
		# find the padding from our numbers
		if info[key]['frame_numbers']:
			...
			#print "\t%s (%d-%d)" % (file_template,start,end)
			info[key]['pad'] = pad
			info[key]['start'] = start
			info[key]['end'] = end
			info[key]['template'] = file_template
		#else:
			#print "\t[Single] %s%s" % (key)
...
def build_rv_file(filename,path,sequences,fps=24):
	...
	# get the max end frame number for the header
	starts = []
	ends = []
	...
	for k in sequences.keys():
		...
		if event_no == 10:
			break
	# string movie = "/hosts/opal/bipool/nas0/CGI/12A191_target_holiday_2012/shot/helpers/help060/work.ryan/maya/images/help060_c02_a22_l08_s08/help060_c02_a22_l08_s08_bty_cop.1-67#.exr"
