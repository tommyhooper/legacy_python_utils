...
class Photomatix:
	...
	def create_dest_dir(self):
		...
		if not os.path.exists(self.dest_path):
			...
			try:
				fileutil.makedirs(self.dest_path,16895)
			except:pass
...
if __name__ == '__main__':
		...
		for source_path in sorted(search):
			...
			for group in groups(images,3):
				...
				if frame_count < start_frame:
					print "Skipping frame %04d.exr (start frame set to %s)" % (frame_count,start_frame)
					continue
				outfile = '%s/%04d.exr' % (dest_path,frame_count)
	...
	# for each of the threads to catch and exit
	...
	# updating progress as we go
	while not queue.empty():
		crnt = frame_count - queue.qsize()
		if crnt > frame_count:
			crnt = frame_count
		pct = int(round(crnt/float(frame_count)*100))
		print "  Progress: %d of %d [%d%%]\r" % (crnt,frame_count,pct),
		sys.stdout.flush()
		time.sleep(1)

	# queue is empty block on each thread 
	# till they all exit
