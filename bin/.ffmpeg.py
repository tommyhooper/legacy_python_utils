...
def get_hoc_h264_ffmpeg(outfile,count,audio=None,title=None,threads=2):
	"""
...
def get_hoc_prores_ffmpeg(outfile,count,audio=None,threads=2):
	"""
	Return the ffmpeg command to create an
...
def get_hoc_netflix_ffmpeg(outfile,count,audio=None,threads=2):
	"""
...
def convert(infile,start,end,threads,slate=False):
	...
	width,height = commands.getoutput(identify).split(':')
...
if __name__ == '__main__':
	...
	convert(infile,start,end,ffmpeg_threads,slate=True)
