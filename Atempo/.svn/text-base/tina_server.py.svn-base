...
#### THREAD EXAMPLE ####
directories = ['/tmp','/usr/bin','/usr/games','/usr/share']
queue = Queue.Queue()
...
class tina_thread(object):
	...
	def add_job(self,job_type,project_list,close=False,logfile='/tmp/rsync_log',dry_run=False,log_message=None):
		"""
		Add a tina job to the queue.
		"""
		entry = {	'type':job_type,
	...
	def start(self):
		#print "Starting tina thread"
		threading.Thread(target=self._start).start()
		self.running = True

	def stop(self):
		#print "Stopping tina thread"
		self._stopped = true
...
class TinaServer:
	...
	def create_thread(self):
		...
		new_thread = rsync_thread(next_thread_num)
...
#			self.thread.stop()
	...
	def status(self):
		"""
		Get the status from the tina thread and put it
		in the progress file
		"""
		status = self.thread.status()
#		print_array(status)
		message = ["------ THREAD PROGRESS ------ "]
		if self.log_message:
...
if __name__ == '__main__':
	r = TinaServer()
	t1 = r.create_thread()
	t1.add_job('archive','project_name_01',close=False,dry_run=True)
	t1.add_job('archive','project_name_02',close=True,dry_run=True)
	t1.add_job('restore','project_name_03',dry_run=True)
	t1.add_job('delete','project_name_04',close=False,dry_run=True)
	t1.add_job('delete','project_name_05',close=True,dry_run=True)
	r.print_queue()
